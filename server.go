...
func (srv *Server) serveUDP(conn *net.UDPConn) error {
	...
	for {
		...
		if srv.Encryption {
			...
			// message (which prerequisites are that we're current state is XX2
			...
			if toSend != nil {
				...
				if ns.PipeState != READY && !ns.Initiator {
					connUDP.SetCoapHeaders(buf, nil, ns.PipeState-1, mID)
				} else {
					connUDP.SetCoapHeaders(buf, nil, ns.PipeState, mID)
				}
